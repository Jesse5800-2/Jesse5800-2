...
export const fontsList: FontDefinition[] = [
	{
  name: "Fusion Pixel 12px Proportional SC",  // Fontsource 上的标准名称
  cssVariable: "--font-fusion-pixel",          // 你自己定义的 CSS 变量名
  provider: "fontsource",                      // 使用 Fontsource 提供器
  weights: ["400"],                            // 该字体只有常规字重
  styles: ["normal"],
  subsets: ["latin"],                          // 包已含 SC 子集，这里写 latin 即可
  fallbacks: ["sans-serif"],
	}, 
	{
		name: "Zen Maru Gothic",
		cssVariable: "--font-zen-maru-gothic",
		// 使用 npm 提供器：直接读取本地安装的 @fontsource/zen-maru-gothic 包，
		// 避免 fontsource 元数据 API（api.fontsource.org）在部分网络/地区不可达时
		// 被静默吞掉并报 “No data was found for the --font-zen-maru-gothic family”。
		// 注意：npm 提供器会读取包内 index.css 的全部字重，weights/subsets 字段仅作参考。
		provider: "npm",
...
export const fontConfig: FontSelectionConfig = {
	...
	selected: ["--font-fusion-pixel","--font-zen-maru-gothic"], 
	...
	// 主页横幅副标题字体
	bannerSubtitleFont: "--font-fusion-pixel",
	// 导航栏标题字体
	navbarTitleFont: "",
	// 代码块字体（用于代码高亮和等宽字体场景）
